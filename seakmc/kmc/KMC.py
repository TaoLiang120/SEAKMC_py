...
from seakmc.input.Input import SP_COMPACT_HEADER, SP_KMC_HEADER, SP_DATA_HEADER
...
class Basin():
    def __init__(
        ...
        if self.nSP <= 0:
            raise ValueError("No saddle point in KMC step!")
...
class SuperBasin():
    ...
    def compute_mean_residence_time(self, idnow):
        ...
        '''
        print("inside mean residence time")
        print(f"transit:{self.transit}")
        print(f"indelta:{inDelta} occu:{np.dot(inDelta, ainv.T)}")
        print(f"mrtime:{self.mrtime} sumtime:{self.sumtime}")
        print("========")
        '''
                    ...
                    if idx != -1:
                        ...
                        for isp in range(ispstart, ispend):
                            if isp in thisBasin.localisp:
                                ...
                                if abs(thisbarr-self.bbarr)>=Tol4Barr: pass
                                else:
                                    ...
                                    if maxdiff <= Tol4Disp:
                                        ...
                                        thisBasin.nSP -= 1
                                        self.update_transition_matrix(iblast, thisBasin.id, ForwardRate, br, thisBasin.sumfreq)
                                        ##print("found backward sp")
                                        ##print(f"iblast:{iblast} ibthis:{thisBasin.id} isp:{isp} ind:{ind}")
                                        ##print("+++++++++")
                            else: pass
                    potAVs[iav]=True
    ...
    def check_isVisit(self, iba, iav, isp, Tol4Disp=0.1, Tol4Barr=0.03):
        isVisit=False
        if self.nBasin<=2: pass
        else:
            thisBasin = self.Basin_list[iba]
            idav = thisBasin.idavs[iav]
            thisdata = copy.deepcopy(thisBasin.data)
            thisdata.update_coords_from_disps(idav, (thisBasin.fdisps[isp]).astype(float), thisBasin.AVitags, Reset_MolID=False)
            ...
            for ib in range(self.nBasin):
                if isVisit: pass
                    ...
                    if iba==ib: pass
                        ...
                        if maxdiff <= Tol4Disp:
                            ##print("ISVISIT")
                            ##print(f"transit before:{self.transit}")
                            self.remove_backward_SP(bas, Tol4Disp=Tol4Disp, Tol4Barr=Tol4Barr, ForwardRate = self.frate, Handle_no_Backward="STAYIN")
                            self.now = bas.id
                            isVisit = True
                            ##print(f"transit after:{self.transit}")
                            ##print("======")
...
class DataKMC():
    ...
    def initialization(self, thisSuperBasin, thisBasin):
        ...
        isBackward = thisSuperBasin.remove_backward_SP(thisBasin, \
                Tol4Disp=self.sett["Tol4Disp"], Tol4Barr=self.sett["Tol4Barr"], ForwardRate = "NA", Handle_no_Backward=self.sett["Handle_no_Backward"])
        if not isBackward:
    ...
    def select_event(self, thisSuperBasin):
        ...
        if rnd[0] == 0.0: isel = 0
        else: isel = sel[0][sel[0].shape[0]-1]
        return isel
    ...
    def update_coords4relaxation(self, thisSuperBasin):
        ...
        if self.sett["DispStyle"][0:2].upper() == "FI":
            ...
            thisdata.update_coords_from_disps(idav, thisBasin.disps[isp].astype(float), thisBasin.AVitags, Reset_MolID=False)
        return thisdata

    def Prob_to_file(self, filename, DataSPs, DetailOut=False, fsel="SP_Details.csv", SPs4Detail="Auto", VerySmallNumber=1.0e-20):
        ...
        def get_disp_details(arraylist, isp, DataSPs, df_SPs, DispStyle="SP"):
            ...
            if DispStyle == "FI": x = DataSPs.fdisps[isp].copy()
            elif DispStyle == "FS": x = DataSPs.fdisps[isp].copy() - DataSPs.disps[isp]
            else: x = DataSPs.disps[isp].copy()
        ...
        ## absorb applied to basins
        #isAbsorbSPs = self.isAbsorb[iavs]
        ## absorb applied to saddle points
