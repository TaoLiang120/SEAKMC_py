...
from seakmc.input.Input import SP_COMPACT_HEADER, SP_COMPACT_HEADER4Delete, DEFECTBANK_ATOMS_HEADER, DEFECTBANK_DISPS_HEADER
from seakmc.input.Input import SP_DATA_HEADER, NDISPARRAY, SEQUENCE_DISPARRAY, NENTRY_COMPACT_DISP
...
class SaddlePoint():
    ...
    def array_ddisplacement_info(self, x, SumAbs=False, AbsVal=True):
        if SumAbs: dsums = np.sum(np.absolute(x), axis=1)
        else: dsums = np.sum(x, axis=1)
    ...
    def get_disp_value(self, Str="SP", Dtype="DMAG", AbsVal=True):
        ...
        if Dtype == "DMAG": thisval = mat_mag(dmags)
        elif Dtype == "DMAGX": thisval = dmags[0]
        elif Dtype == "DMAGY": thisval = dmags[1]
        elif Dtype == "DMAGZ": thisval = dmags[2]
        elif Dtype == "DMAG_DMIN": thisval = np.argmin(dmags)
        elif Dtype == "DMAG_DMAX": thisval = np.argmax(dmags)
        elif Dtype == "DMAG_DRX": thisval = abs(dmags[0])/max(mat_mag(dmags), 1.0e-6)
        elif Dtype == "DMAG_DRY": thisval = abs(dmags[1])/max(mat_mag(dmags), 1.0e-6)
        elif Dtype == "DMAG_DRZ": thisval = abs(dmags[2])/max(mat_mag(dmags), 1.0e-6)
        elif Dtype == "DMAG_DRXY": thisval = abs(dmags[0])/max(abs(dmags[1]), 1.0e-6)
        elif Dtype == "DMAG_DRXZ": thisval = abs(dmags[0])/max(abs(dmags[2]), 1.0e-6)
        elif Dtype == "DMAG_DRYZ": thisval = abs(dmags[1])/max(abs(dmags[2]), 1.0e-6)
        elif Dtype == "DSUM": thisval = mat_mag(dsums)
        elif Dtype == "DSUMX": thisval = dsums[0]
        elif Dtype == "DSUMY": thisval = dsums[1]
        elif Dtype == "DSUMZ": thisval = dsums[2]
        elif Dtype == "DSUM_DMIN": thisval = np.argmin(dsums)
        elif Dtype == "DSUM_DMAX": thisval = np.argmax(dsums)
        elif Dtype == "DSUM_DRX": thisval = abs(dsums[0])/max(mat_mag(dsums), 1.0e-6)
        elif Dtype == "DSUM_DRY": thisval = abs(dsums[1])/max(mat_mag(dsums), 1.0e-6)
        elif Dtype == "DSUM_DRZ": thisval = abs(dsums[2])/max(mat_mag(dsums), 1.0e-6)
        elif Dtype == "DSUM_DRXY": thisval = abs(dsums[0])/max(abs(dsums[1]), 1.0e-6)
        elif Dtype == "DSUM_DRXZ": thisval = abs(dsums[0])/max(abs(dsums[2]), 1.0e-6)
        elif Dtype == "DSUM_DRYZ": thisval = abs(dsums[1])/max(abs(dsums[2]), 1.0e-6)
        elif Dtype == "DSUM_RABS": thisval = mat_mag(dsums)/max(mat_mag(absdsums), 1.0e-6)
        elif Dtype == "DSUMX_RABS": thisval = dsums[0]/max(absdsums[0], 1.0e-6)
        elif Dtype == "DSUMY_RABS": thisval = dsums[1]/max(absdsums[1], 1.0e-6)
        elif Dtype == "DSUMZ_RABS": thisval = dsums[2]/max(absdsums[2], 1.0e-6)
        elif Dtype == "DMAX": thisval = mat_mag(dmaxs)
        elif Dtype == "DMAXX": thisval = dmaxs[0]
        elif Dtype == "DMAXY": thisval = dmaxs[1]
        elif Dtype == "DMAXZ": thisval = dmaxs[2]
        elif Dtype == "DMAX_DMIN": thisval = np.argmin(dmaxs)
        elif Dtype == "DMAX_DMAX": thisval = np.argmax(dmaxs)
        elif Dtype == "DMAX_DRX": thisval = abs(dmaxs[0])/max(mat_mag(dmaxs), 1.0e-6)
        elif Dtype == "DMAX_DRY": thisval = abs(dmaxs[1])/max(mat_mag(dmaxs), 1.0e-6)
        elif Dtype == "DMAX_DRZ": thisval = abs(dmaxs[2])/max(mat_mag(dmaxs), 1.0e-6)
        elif Dtype == "DMAX_DRXY": thisval = abs(dmaxs[0])/max(abs(dmaxs[1]), 1.0e-6)
        elif Dtype == "DMAX_DRXZ": thisval = abs(dmaxs[0])/max(abs(dmaxs[2]), 1.0e-6)
        elif Dtype == "DMAX_DRYZ": thisval = abs(dmaxs[1])/max(abs(dmaxs[2]), 1.0e-6)
        elif Dtype == "VMAX": thisval = mat_mag(vmaxs)
        elif Dtype == "VMAXX": thisval = vmaxs[0]
        elif Dtype == "VMAXY": thisval = vmaxs[1]
        elif Dtype == "VMAXZ": thisval = vmaxs[2]
        elif Dtype == "VMAX_DMIN": thisval = np.argmin(vmaxs)
        elif Dtype == "VMAX_DMAX": thisval = np.argmax(vmaxs)
        elif Dtype == "VMAX_DRX": thisval = abs(vmaxs[0])/max(mat_mag(vmaxs), 1.0e-6)
        elif Dtype == "VMAX_DRY": thisval = abs(vmaxs[1])/max(mat_mag(vmaxs), 1.0e-6)
        elif Dtype == "VMAX_DRZ": thisval = abs(vmaxs[2])/max(mat_mag(vmaxs), 1.0e-6)
        elif Dtype == "VMAX_DRXY": thisval = abs(vmaxs[0])/max(abs(vmaxs[1]), 1.0e-6)
        elif Dtype == "VMAX_DRXZ": thisval = abs(vmaxs[0])/max(abs(vmaxs[2]), 1.0e-6)
        elif Dtype == "VMAX_DRYZ": thisval = abs(vmaxs[1])/max(abs(vmaxs[2]), 1.0e-6)
        else:
            raise ValueError("Unrecognized displacement type for validation!")
            quit()
        return thisval
    ...
    def get_SPs_from_symmetry(self, coords, symmOPs):
        ...
        for i in range(symmOPs.nOP):
            if mat_mag(symmOPs.OPs[i].rotation_matrix-IDENTITY) <= self.tol: pass
            else:
                thisdisp, thisfdisp, isValid = self.get_disp_from_opmat(coords, symmOPs.OPs[i].rotation_matrix, radiusmin)
                if isValid:
                    isDup = False
                    ...
                    if not isDup:
                        ...
                        thissp = SaddlePoint(self.idav, self.idsps, self.type, self.barrier, self.prefactor, self.ebias, self.isconnect, \
...
class AV_SPs():
    ...
    def get_min_bar_dmag(self, barriercut=1000000):
        ...
        for i in range(self.nSP):
            if not typemask[self.SPlist[i].type-1]:
                ...
                typemask[self.SPlist[i].type-1] = True
    ...
    def append_SP2compact_arrays(self, thisSP, arraylist):
        ...
        arraylist[2] = np.append(arraylist[2], [thisSP.type])
    ...
    def get_compact_df(self, arraylist):
        marray = zip(arraylist[0], arraylist[1], arraylist[2], arraylist[3], arraylist[4], arraylist[5], \
                     ...
                     arraylist[12], arraylist[13], arraylist[14].astype(int), arraylist[15], arraylist[16], arraylist[17], \
                     arraylist[18], arraylist[19], arraylist[20].astype(int), arraylist[21], arraylist[22], arraylist[23], \
    ...
    def to_dataframe(self, iSPs="All"):
        if isinstance(iSPs, int): iSPs = np.array([iSPs])
        elif isinstance(iSPs, list) or isinstance(iSPs, np.ndarray): iSPs = np.array(iSPs).astype(int)
        else: iSPs=np.arange(self.nSP, dtype=int)
    ...
    def remove_small_barrier(self, minbarrier=0.0):
        ...
        for i in range(self.nSP):
            if not typemask[self.SPlist[i].type-1]:
                if self.SPlist[i].barrier < minbarrier:
                    todel.append(i)
                    typedelmask[self.SPlist[i].type-1] = True
                typemask[self.SPlist[i].type-1] = True
            else:
                if typedelmask[self.SPlist[i].type-1]: todel.append(i)
        ...
        else: df_delete_this = pd.DataFrame(columns=SP_COMPACT_HEADER4Delete)
    ...
    def get_Delete_Disp(self, thisSP, Conn=False):
        ...
        if self.sett["ValidSPs"]["NScreenDisp"]>0:
            if Conn:
                if self.sett["ValidSPs"]["toScreenDisp"].upper()=="ALL" or self.sett["ValidSPs"]["toScreenDisp"].upper()=="CONN": if_Check=True
            else:
                if self.sett["ValidSPs"]["toScreenDisp"].upper()=="ALL" or self.sett["ValidSPs"]["toScreenDisp"].upper()=="NOTCONN": if_Check=True
        if if_Check:
            Delete_Disp = False
            for idisp in range(self.sett["ValidSPs"]["NScreenDisp"]):
                thisval = thisSP.get_disp_value(Str=self.sett["ValidSPs"]["ScreenDisp"]["Str4ScreenD"][idisp], \
                        Dtype=self.sett["ValidSPs"]["ScreenDisp"]["Type4ScreenD"][idisp], AbsVal = self.sett["ValidSPs"]["ScreenDisp"]["AbsVal4ScreenD"][idisp])
                if self.sett["ValidSPs"]["ScreenDisp"]["AND4ScreenD"][idisp] and not Delete_Disp:
                    isInside = self.this_screen_condition(thisval, self.sett["ValidSPs"]["ScreenDisp"]["MinVal4ScreenD"][idisp], self.sett["ValidSPs"]["ScreenDisp"]["MaxVal4ScreenD"][idisp], NA_to = False)
                    if isInside: Delete_Disp = True
                if Delete_Disp and not self.sett["ValidSPs"]["ScreenDisp"]["AND4ScreenD"][idisp]:
                    isRevoke= self.this_screen_condition(thisval, self.sett["ValidSPs"]["ScreenDisp"]["MinVal4ScreenD"][idisp], self.sett["ValidSPs"]["ScreenDisp"]["MaxVal4ScreenD"][idisp], NA_to = False)
    ...
    def get_Delete_Eng(self, thisSP, Conn=False):
        ...
        if self.sett["ValidSPs"]["NScreenEng"]>0:
            if Conn:
                if self.sett["ValidSPs"]["toScreenEng"].upper()=="ALL" or self.sett["ValidSPs"]["toScreenEng"].upper()=="CONN": if_Check=True
            else:
                if self.sett["ValidSPs"]["toScreenEng"].upper()=="ALL" or self.sett["ValidSPs"]["toScreenEng"].upper()=="NOTCONN": if_Check=True
        if if_Check:
            Delete_Eng = False
            for ieng in range(self.sett["ValidSPs"]["NScreenEng"]):
                ...
                if self.sett["ValidSPs"]["ScreenEng"]["AND4ScreenE"][ieng] and not Delete_Eng:
                    isInside = self.this_screen_condition(thisval, self.sett["ValidSPs"]["ScreenEng"]["MinVal4ScreenE"][ieng], self.sett["ValidSPs"]["ScreenEng"]["MaxVal4ScreenE"][ieng], NA_to = False)
                    if isInside: Delete_Eng = True
                if Delete_Eng and not self.sett["ValidSPs"]["ScreenEng"]["AND4ScreenE"][ieng]:
                    isRevoke = self.this_screen_condition(thisval, self.sett["ValidSPs"]["ScreenEng"]["MinVal4ScreenE"][ieng], self.sett["ValidSPs"]["ScreenEng"]["MaxVal4ScreenE"][ieng], NA_to = False)
                    if isRevoke: Delete_Eng = False
        return Delete_Eng

    def valid_SPs(self, arraylist, reasons, Delete = True):
        if Delete:
            if self.sett["ValidSPs"]["CheckConnectivity"] or self.sett["ValidSPs"]["NScreenDisp"]>0 or self.sett["ValidSPs"]["NScreenEng"]>0:
            ...
            if self.nSP <= self.coords.shape[1]:
                ...
                for i in range(nstart, self.nSP-1):
                    for j in range(i+1, self.nSP):
                        if not thismask[j]:
                            isDup = False
                            if abs(self.SPlist[i].barrier-self.SPlist[j].barrier) < self.sett["ValidSPs"]["EnTol4AVSP"]:
                                isDup = self.is_duplicate(self.SPlist[i].disp, self.SPlist[j].disp, Tolerance= self.sett["ValidSPs"]["Tol4AVSP"])
                            if isDup:
                                todel.append(j)
                                thismask[j] = True
            else:
    ...
    def group_saddles(self, group_by = "dmat", SPlist = None):
        ...
        for i in range(nSP):
                    ...
                    if idjsp == 0:
                        ...
                        if group_by == "dmat":
                            if abs(SPlist[isp].barrier-SPlist[j].barrier) < self.sett["ValidSPs"]["EnCut4GSP"] and \
                               abs(SPlist[isp].dlen[0]-SPlist[j].dlen[0]) < self.sett["ValidSPs"]["MagCut4GSP"] and \
                               abs(SPlist[isp].dlen[1]-SPlist[j].dlen[1]) < self.sett["ValidSPs"]["MagCut4GSP"] and \
                               abs(SPlist[isp].dlen[2]-SPlist[j].dlen[2]) < self.sett["ValidSPs"]["MagCut4GSP"] and \
                               abs(SPlist[isp].dmag-SPlist[j].dmag) < self.sett["ValidSPs"]["MagCut4GSP"]:
                                if angle < self.sett["ValidSPs"]["AngCut4GSP"] and \
                                   ...
                                   angles[2] < self.sett["ValidSPs"]["AngCut4GSP"]:
                                    isgroup = True
                        if group_by == "type":
                            if SPlist[isp].type == SPlist[j].type: isgroup = True
                            else:
                                ...
                                jangs = jangs[aj]
                                if abs(SPlist[isp].barrier-SPlist[j].barrier) < self.sett["ValidSPs"]["EnCut4Type"] and \
                                   abs(SPlist[isp].dmag-SPlist[j].dmag) < self.sett["ValidSPs"]["MagCut4Type"] and \
                                   ...
                                   abs(ilens[2]-jlens[2]) < self.sett["ValidSPs"]["LenCut4Type"]:
                                       ...
                                       for ii in range(iangs.shape[0]):
                                           if abs(iangs[ii]-jangs[ii]) > self.sett["ValidSPs"]["AngCut4Type"] and \
                                               abs(iangs[ii]+jangs[ii]-180.0) > self.sett["ValidSPs"]["AngCut4Type"]:
                                               if_angle = False
                                               break
            ...
            if idthis < nSP:
                if if_loop == 0: chain_id[idthis] = inext
                else:
        ...
        if len(nlist) == 0: pass
            ...
            for j in range(i+1, len(avSPs)):
                ...
                if jdav not in nlist: pass
                    ...
                    ijcom, icomind, jcomind, isCommon = find_common_atoms_AVs(AVitags, idav, jdav, iavna, javna, ncommonmin=thisnmin)
                    if isCommon:
                        for ii in range(avSPs[i].nSP):
                            ...
                            if isCheck:
                                for jj in range(avSPs[j].nSP):
                                    jsp = avSP[j].SPlist[jj]
                                    jdisp =  jsp.disps.copy()
                                    jdisp = jdisp[:,0:javna]
                                    jtol = min(SPsett["ValidSPs"]["Tol4Disp"], jsp.dmax*SPsett["ValidSPs"]["R2Dmax4Tol"])
...
class DefectBank():
    ...
    @classmethod
    def from_AV_SPs(cls, data, AV_SPs, id = None, sch_symbol="C1", scaling=1.0, namax=70, namin=10, Style='Type', significant_figures=6):
        if data.nactive<namin: return False
        else:
            ...
            return(cls(id, atoms, disps, sch_symbol=sch_symbol, namax=namax, namin=namin, significant_figures=significant_figures))

    @classmethod
    def from_files(cls, id, fheader, sch_symbol="C1", filepath=None, namax=70, namin=10, SortDisps= False, significant_figures=6):
        if isinstance(filepath, str): pass
        else: filepath = os.getcwd()
        ...
        if thisna>=namin:
            ...
            if len(disps)>0:
                ...
                thisDB = cls(id, atoms, disps, sch_symbol=sch_symbol, namax=namax, namin=namin, significant_figures=significant_figures)
        return thisDB
