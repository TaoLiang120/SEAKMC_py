...
class SPSearch():
    def __init__(
        ...
        self.dmAV = dmAV
        if SNC:
            if isinstance(self.dmAV, DynMat) or isinstance(self.dmAV, VibMat): pass
            else:
                raise ValueError("SPSearch must have DynMat object of the active volume.")
        self.pre_disps = pre_disps
        ...
        for fixtype in self.sett.spsearch["FixTypes_dict"]:
            for i in range(3):
                if i in self.sett.spsearch["FixTypes_dict"][fixtype]:
                    self.Scaling4Types[i] = np.select([types==fixtype, types!=fixtype],[0.0, self.Scaling4Types[i]])
                else:
                    pass
    ...
    def generate_step_scale(self, x, r4zero=0.65, MinSpan=2.0):
        ...
        if isinstance(MinSpan, float) or isinstance(MinSpan, int):
            if xmax-xmin<MinSpan: isRescale = False
        if isRescale:
            if isinstance(r4zero,str): s0=np.mean(x)
            else: s0 = xmin+(xmax-xmin)*r4zero
    ...
    def get_AtomSquence(self, xyz, sort_by="DXYZ", AbsVal=True):
        ...
        if sort_by.upper() == "DXYZ":
            ...
            if AbsVal: thisx = np.absolute(x)
            else: thisx = x
        elif sort_by.upper() == "Y":
            if AbsVal: thisx = np.absolute(y)
            else: thisx = y
        elif sort_by.upper() == "Z":
            if AbsVal: thisx = np.absolute(z)
            else: thisx = z
    ...
    def generate_VN(self):
        ...
        else: VN = np.random.rand(3, self.nactive)-0.5
        return VN
        ...
        if self.DMAG > self.DMAGCUT:
            ...
            for i in range(thisSPS.nSP):
                if thisSPS.SPlist[i].disp.shape[1]-self.XDISP.shape[1] > 0:
                    ...
                    angle = mats_angle(self.XDISP[:,0:thisSPS.SPlist[i].disp.shape[1]], thisSPS.SPlist[i].disp, Flatten=True)
                else: angle = mats_angle(self.XDISP, thisSPS.SPlist[i].disp, Flatten=True)
    ...
    def reprepare_runner(self):
        ...
        [total_energy, coords, isValid, errormsg] = self.force_evaluator.init_spsearch_runner(thisdata, self.thiscolor, self.nactive)
        if not isValid:
            print(errormsg)
            quit()
        thisdata = None
        ...
        if isValid:
            for i in range(thisSPS.nSP):
                if abs(self.BARR-thisSPS.SPlist[i].barrier) < self.sett.saddle_point["ValidSPs"]["EnTol4AVSP"]:
                    ...
                    if dmax < self.sett.saddle_point["ValidSPs"]["Tol4AVSP"]:
                        isValid = False
            ...
            if not toDel:
                if self.BARR < self.sett.saddle_point["BarrierMin"] or \
                   ...
                   self.EBIAS > self.EBIASCUT:
                    ...
                    if isinstance(self.sett.saddle_point["EbiasMin"], float) and self.EBIAS < self.sett.saddle_point["EbiasMin"]:
                        toDel = True
        else: toDel=True
        return toDel
    ...
    def calculate_prefactor(self):
        ...
        if self.ISVALID and self.dmAV is not None:
            ...
            thisdata = copy.deepcopy(self.data)
            thisdata.update_coords_from_disps(self.XDISP)
            [encalc, coords, isValid, errormsg] = self.force_evaluator.run_runner("SPSDYNMAT", thisdata, self.thiscolor, nactive = self.data.nactive)
            if not isValid:
                print(errormsg)
                quit()
            thisdata = None
            ...
            vmSP= VibMat.from_file("Runner_"+str(self.thiscolor)+"/dynmat.dat", id = self.thiscolor, delimiter = delimiter, vibcut = vibcut, LowerHalfMat=LowerHalfMat)
...
class Dimer(SPSearch):
    ...
    def dimer_init(self, thisVN=None):
        ...
        mask = np.hstack((np.ones(self.data.nactive, dtype=bool), np.zeros(self.data.nbuffer, dtype=bool), np.zeros(self.data.nfixed, dtype=bool)))
        atoms = atoms_array[mask]
        self.X0 = np.vstack((atoms["x"], atoms["y"], atoms["z"])) #/self.Rescale
        self.XACT = np.copy(self.X0)

        mask = np.hstack((np.zeros(self.data.nactive, dtype=bool), np.ones(self.data.nbuffer, dtype=bool), np.zeros(self.data.nfixed, dtype=bool)))
        atoms = atoms_array[mask]
        XBUF =  np.vstack((atoms["x"], atoms["y"], atoms["z"]))
        mask = np.hstack((np.zeros(self.data.nactive, dtype=bool), np.zeros(self.data.nbuffer, dtype=bool), np.ones(self.data.nfixed, dtype=bool)))
        atoms = atoms_array[mask]
        ...
        if thisVN is not None: self.VN = thisVN.copy()
        else: self.VN = self.generate_VN()

        '''
        fthisVN = "VN_"+str(self.idsps)+".dat"
        if os.path.isfile(fthisVN):
            self.VN = np.loadtxt(fthisVN)
            self.VN = self.VN[:, 0:168]
        ...
        self.iter = 0
        [total_energy, coords, isValid, errormsg] = self.force_evaluator.init_spsearch_runner(self.data, self.thiscolor, self.nactive)
        if not isValid:
            print(errormsg)
            quit()
    ...
    def update_translation_step(self, itstep):
        ...
        if self.DECAYSTYLE[0:3].upper() == "FIX": pass
        else:
            drate = get_decayrate(itstep)
            self.TSTEPF = (self.sett.spsearch["TrialStepsize"]*drate + self.sett.spsearch["MinStepsize"]) #/self.Rescale
            self.TSTEPM = (self.sett.spsearch["MaxStepsize"]*drate + self.sett.spsearch["MinStepsize"]*self.sett.spsearch["RatioStepsize"]) #/self.Rescale
    ...
    def move_atoms_to_predisp(self):
        ...
        if ndiff>0:
            ...
            if self.sett.spsearch["HandleVN"]["ResetVN04Preload"]:
                self.VN = self.VN*self.sett.spsearch["HandleVN"]["RatioVN04Preload"] + np.hstack((self.pre_disps, zerosapp))
        else:
            thisdisp = self.pre_disps[:, 0:self.X0.shape[1]]
            if self.FixType: thisdisp = thisdisp*self.Scaling4Types
            self.X0 = self.X0 + thisdisp
            if self.sett.spsearch["HandleVN"]["ResetVN04Preload"]:
                self.VN = self.VN*self.sett.spsearch["HandleVN"]["RatioVN04Preload"] + self.pre_disps[:, 0:self.X0.shape[1]]
        ...
        if "RAS" in self.RescaleValue:
            ...
            if "SIG" in self.sett.spsearch["HandleVN"]["RescaleStyle4RAS"].upper():
                self.RescaleRAS = self.generate_sigmoid_scale(RAS, range=self.sett.spsearch["HandleVN"]["XRange4RAS"], \
                        r4zero=self.Ratio4Zero4RAS, MinSpan=self.sett.spsearch["HandleVN"]["MinSpan4RAS"])
            else:
    ...
    def compute_xRescale_logdisps(self):
        if "SUM" in self.sett.spsearch["HandleVN"]["RescaleValue"].upper():
            ...
            if self.SNC: d = self.convert_coord_backward(self.VN)
            else: d = self.VN.copy()
    ...
    def compute_Rescale(self):
        if "LOG" in self.RescaleValue:
            if "SIG" in self.sett.spsearch["HandleVN"]["RescaleStyle4LOGV"].upper():
                self.RescaleLOGV = self.generate_sigmoid_scale(self.xRescale, range=self.sett.spsearch["HandleVN"]["XRange4LOGV"], \
                    r4zero=self.Ratio4Zero4LOGV, MinSpan=self.sett.spsearch["HandleVN"]["MinSpan4LOGV"])
            else:
            ...
            if "RAS" in self.RescaleValue:
                if self.sett.spsearch["HandleVN"]["TakeMin4MixedRescales"]:
                    self.Rescale = np.select([self.RescaleLOGV<=self.RescaleRAS, self.RescaleLOGV>self.RescaleRAS],[self.RescaleLOGV, self.RescaleRAS])
                else:
                    self.Rescale = np.select([self.RescaleLOGV<=self.RescaleRAS, self.RescaleLOGV>self.RescaleRAS],[self.RescaleRAS, self.RescaleLOGV])
            else: self.Rescale = self.RescaleLOGV.copy()
        else:
    ...
    def dimer_force(self):
        if self.SNC: self.XCR = self.convert_coord_backward(self.XC)
        else: self.XCR = self.XC.copy()
        self.XA = np.hstack((self.XCR, self.XNOT))

        [encalc, FC, isValid, errormsg] = self.force_evaluator.get_spsearch_forces(self.XA.T, self.data, self.thiscolor, self.nactive)
        if not isValid:
            print(errormsg)
            quit()
        ...
        self.FC = (FC.reshape([self.nactive,3])).T  #*self.Rescale
    ...
    def dimer_update(self):
        ...
        self.FN = 0.5/self.RDIMER*(self.F1 - self.F2 - np.multiply(self.VN, mats_sum_mul(self.F1, self.VN) - mats_sum_mul(self.F2, self.VN)))
    ...
    def dimer_rotate(self):
        if self.RO_NEW:
            ...
            if self.RO_FWD:
                ...
                self.dimer_projection(TR_HOR=self.TR_HOR)
            if self.RO_IDLE: self.RO_OPT=True
            else:
                if self.FNUNIT < self.sett.spsearch["FMin4Rot"]:
                    ...
                    if self.RO_FWD:
                        if self.NROITR > self.sett.spsearch["NMax4Rot"]: self.RO_OPT = True
                        elif self.ROFNRS < self.sett.spsearch["FThres4Rot"]: self.RO_OPT = True
                        self.dimer_projection(TR_HOR=self.TR_HOR)
            if self.RO_OPT:
                ...
                if self.isTR_HOR and self.TRCURV<-0.01 and self.ROCURV>0.01:
                    if self.isInitWell: self.TR_HOR = False
                    else:
                        if self.EDIFF>self.sett.spsearch["En4TransHorizon"]: self.TR_HOR=True
                        else: self.TR_HOR = False
                else:
                    self.TR_HOR = False
                ...
                if self.NTSITR % 10 == 0:
                    ...
                    print(f"ROCURV: {self.ROCURV} TRCURV: {self.TRCURV} TR_HOR: {self.TR_HOR} IB:{self.isInitWell}")
                    print('===========')
                '''
    ...
    def dimer_search(self, thisSPS, ReSearch = False):
        if self.sett.spsearch["ShowIterationResults"]:
            ...
            #summary_cols = ["iter", "ntrans", "curve", "ediff", "ediffmax", "sumx", "sumy", "sumz"]
        ...
        while not self.DI_CON:
            ...
            if ftotal<self.FCONV: self.FO_CON = True
            else:  self.FO_CON = False
            if self.NTSITR > self.IgnoreTransSteps: self.ST_CON = True
            if self.FO_CON and self.ST_CON: self.DI_CON = True

            if self.ST_CON and self.sett.spsearch["CheckAng"] and self.NTSITR % self.sett.spsearch["CheckAngSteps"] == 0:
            ...
            if self.sett.spsearch["ShowIterationResults"]:
                if self.NTSITR % self.sett.spsearch["Inteval4ShowIterationResults"] == 0:
                    summary_array = np.append(summary_array, [int(self.iter), int(self.NTSITR), round(self.ROCURV,3), round(self.EDIFF,3), round(self.EDIFF_MAX,3)])
                    #disps = self.XCR - self.XACT
                    #dsum = np.sum(disps, axis=1)
                    #summary_array = np.append(summary_array, [int(self.iter), int(self.NTSITR), round(self.ROCURV,3), round(self.EDIFF,3), round(self.EDIFF_MAX,3), dsum[0], dsum[1], dsum[2]])
                    if self.sett.spsearch["ShowVN4ShowIterationResults"]:
                        ...
                        #filename = thisdir+"/instant_displacements_" + str(self.iter)+".csv"
                        df.to_csv(filename)
                    if self.sett.spsearch["ShowCoords4ShowIterationResults"]:
                        ...
                        #filename = thisdir+"/total_displacements_" + str(self.iter)+".csv"
                        df.to_csv(filename)
                    #if self.isRescale: np.savetxt(thisdir+"/RescaleFinal_"+str(self.iter)+".dat", self.Rescale)
    ...
    def dimer_re_search(self, thisSPS, nactive = None):
        ...
        isValid = True
        if ndiff <= 0:
            warnings.warn("The number of atoms for this dimer search is smaller or equal than number of atoms of last one.")
            isValid = False
        if nfixed < 0:
        ...
        if isValid and self.DI_MAX == False:
            ...
            if self.DECAYSTYLE.upper()=="FIXED": pass
            else: self.DECAYSTYLE = "FIXED"
    ...
    def dimer_relaxation(self):
        ...
        def shift_coords(coords):
            ...
            for idim in range(self.data.dimension):
                if self.data.PBC[idim]:
                    ...
                    fcoords[idim] = np.select([diff<-0.5, diff<0.5, diff>=0.5], [fcoords[idim]+1.0, fcoords[idim], fcoords[idim]-1.0])
        ...
        for i in range(len(X12)):
            ...
            self.force_evaluator.close()
            encalc, coords, isValid, errormsg = self.force_evaluator.run_runner("SPSRELAX", thisdata, self.thiscolor, nactive = self.nactive)
            if not isValid:
                print(errormsg)
                quit()
            mask = np.ones(3*self.nactive, dtype = bool)
    ...
    def dimer_finalize(self):
        if self.DI_MAX:
            ...
            if self.sett.spsearch["LocalRelax"]["LocalRelax"]:
                ...
                X1 = self.DRATIO*(self.XCR-self.X0)+self.X0
                X2 = self.DRATIO*(self.X0-self.XCR)+self.X0
                if mat_mag(X1-self.XACT)>= mat_mag(X2-self.XACT): SX = X1.copy()
                else: SX = X2.copy()
                XFIN = SX.copy()
                EFIN = self.ENINIT
                self.ISVALID = True
            ...
            if self.BARR > self.sett.saddle_point["BarrierCut"]: self.ISVALID = False
            elif self.DMAG > self.DMAGCUT: self.ISVALID = False
