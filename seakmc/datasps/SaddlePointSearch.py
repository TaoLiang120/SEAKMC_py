...
from seakmc.core.util import mats_angle, sigmoid_function
from seakmc.spsearch.SPSearch import SPSearch, Dimer
...
def generate_VN(spsearchsett, thisVNS, nactive, SNC=False, dmAV=None):
    ...
    while not isvalid:
        ...
        else: VN = np.random.rand(3, nactive)-0.5
        if spsearchsett["HandleVN"]["CheckAng4Init"]:
            ...
            if anglemin < spsearchsett["HandleVN"]["AngTol4Init"] and thisiter<=spsearchsett["HandleVN"]["MaxIter4Init"]:
                isvalid = False
            else:
                isvalid = True
            thisiter+=1
...
def saddlepoint_search(thiscolor, istep, thissett, idav, thisAV, local_coords, thisSOPs, dynmatAV, SNC, CalPref, \
    ...
    for idsps in range(thisnspsearch):
        ...
        if thisspsearch.ISVALID:
            thissp = SaddlePoint(idav, idsps, idsps+1, thisspsearch.BARR, thisspsearch.PREF, thisspsearch.EBIAS, thisspsearch.ISCONNECT, \
                     ...
                     rdcut = thissett.spsearch["R2Dmax4SPAtom"], dcut=thissett.spsearch["DCut4SPAtom"], dyncut=thissett.spsearch["DynCut4SPAtom"], \
        ...
        tocd = time.time()
        logstr = f"idav:{idav}, idsps: {idsps},  ntrans: {thisspsearch.NTSITR},  barrier:{round(thisspsearch.BARR,float_precision)},  ebias: {round(thisspsearch.EBIAS,float_precision)}"
        logstr += "\n" + f"      dmag:{round(thisspsearch.DMAG,float_precision)}, dmagFin:{round(thisspsearch.FDMAG,float_precision)}, isConnect: {thisspsearch.ISCONNECT}"
        logstr += "\n" + f"      Valid SPs: {thisSPS.nvalid}, num of SPs:{thisSPS.nSP}, time: {round(tocd-ticd, float_precision)} s"
        logstr += "\n" + "-----------------------------------------------------------------"
