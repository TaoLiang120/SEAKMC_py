...
from seakmc.spsearch.SaddlePoints import check_duplicate_avSPs
...
def data_find_saddlepoints(istep, thissett, seakmcdata, DefectBank_list, thisSuperBasin, Eground, \
    ...
    for idav in undo_idavs:
        ...
        if thisAV is not None:
            ...
            logstr += "\n" + f"AV center fractional coords: {round(seakmcdata.defects.at[idav,'xsn'],3), round(seakmcdata.defects.at[idav,'ysn'],3), round(seakmcdata.defects.at[idav,'zsn'],3)}"
            ...
            thisSOPs, isPGSYMM = preSPS.get_SymmOperators(thissett, thisAV, idav, PointGroup = thissett.active_volume["PointGroupSymm"])
            ...
            dynmatAV = None
            if SNC or CalPref:
                SNC, CalPref, dynmatAV = preSPS.get_thisSNC4spsearch(idav, thissett, thisAV, SNC, CalPref, object_dict, thiscolor)

            thisSPS, df_delete_SPs = mySPS.saddlepoint_search(thiscolor, istep, thissett, idav, thisAV, local_coords, thisSOPs, dynmatAV, SNC, CalPref, \
                                          thisSPS, Pre_Disps, thisnspsearch, thisVNS, \
                                          df_delete_SPs, object_dict)

            thisSPS, df_delete_SPs  = postSPS.SPs_1postprocessing(thissett, thisSPS, df_delete_SPs, DFWriter, nSPstart= thisSPS.nSP)
            thisSPS, df_delete_this = DataSPs.check_dup_avSP(idav, thisSPS, seakmcdata.de_neighbors, AVitags, thissett.saddle_point)
            df_delete_SPs = preSPS.update_df_delete_SPs(df_delete_SPs, df_delete_this, DFWriter)

            if thisSPS.nSP > 0:
                if thissett.defect_bank["Recycle"]:
                    DefectBank_list = postSPS.add_to_DefectBank(thissett, thisAV, thisSPS, isRecycled, isPGSYMM, thisSOPs.sch_symbol, DefectBank_list, out_paths[3])
