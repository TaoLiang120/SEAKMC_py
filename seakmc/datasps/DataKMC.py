...
def run_KMC(istep, thisSuperBasin, seakmcdata, AVitags, DataSPs, thissett, simulation_time, thisExports, LogWriter):
    ...
    thisBasin = Basin(len(thisSuperBasin.Basin_list), istep, seakmcdata, AVitags, DataSPs, thissett.kinetic_MC, VerySmallNumber=thissett.system["VerySmallNumber"])
    ...
    logstr = f"Current basin info at {istep} KMC step - ID:{thisBasin.id} Number of saddle points:{nSPthis} ID of selection:{thiskmc.isels}"
    logstr += "\n" + f"                  Selected event - Barrier:{round(forward_barrier, thissett.system['float_precision'])} Backward barrier:{round(backward_barrier,thissett.system['float_precision'])}"
    if thissett.kinetic_MC["AccStyle"][0:3].upper()=="MRM":
        logstr += "\n" + f"Superbasin info at {istep} KMC step - Number of basins:{thiskmc.nBasin} Number of saddle points: {thiskmc.nSP} ID of selection:{thiskmc.gsp_sels}"
        logstr += "\n" + f"Local info of seleted events at {istep} KMC step - Basin ID:{thiskmc.iba_sels} KMC step:{thisSuperBasin.Basin_list[liba].istep} Event ID:{thiskmc.isp_sels}"
        logstr += "\n" + f"All seleted events of the superbasin at {istep} KMC step - Basin ID:{thisSuperBasin.iba_sels} Event ID: {thisSuperBasin.isp_sels}"
    logstr += "\n" + "-----------------------------------------------------------------"
