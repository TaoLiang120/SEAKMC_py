...
from seakmc.core.symmetry import PGSymmOps, SGSymmOps, SymmOP
from seakmc.dynmat.Dynmat import DynMat, VibMat, VibMats
from seakmc.spsearch.SaddlePoints import AV_SPs, Data_SPs, DefectBank
    ...
    for file in os.listdir(DBLoadpath):
        if thisFileHeader in file and ".csv" in file:
            ...
            try:
                ...
                thisDB = DefectBank.from_files(thisid, DBsett["FileHeader"], sch_symbol=thissch_symbol, filepath=DBLoadpath, \
                     namax=DBsett["NMax4DB"], namin=DBsett["NMin4DB"], SortDisps= DBsett["SortDisps"], significant_figures=significant_figures)
...
def get_AV_atom_strain(thisAV, thissett, thiscolor):
    if thisAV is not None:
        if "LAS" in thissett.spsearch["HandleVN"]["RescaleValue"].upper() and thissett.spsearch["HandleVN"]["RescaleVN"]:
            thisStrain = thisAV.estimate_atom_strain(thissett, nactive=thisAV.nactive, nbuffer=thisAV.nbuffer)
        else: thisStrain = None
    else:
        thisStrain = None
    return thisStrain

def initialize_thisSPS(idav, local_coords, nspsearch, thissett):
    thisSPS = AV_SPs(idav, local_coords, [], nspsearch, thissett.saddle_point, float_precision=thissett.system["float_precision"])
    return thisSPS
...
def get_thisSNC4spsearch(idav, thissett, thisAV, thisSNC, thisCalPref, object_dict, thiscolor):
    ...
    if not dynmatAV.isValid:
        ...
        if not dynmatAV.isSNCable:
            thisSNC = False
            if not thisCalPref: dynmatAV=None
    return thisSNC, thisCalPref, dynmatAV

def get_disps_from_DefectBank(thissett, thisAV, thissch_symbol, DefectBank_list, istep):
    thisdisps = []
    if thisAV.nactive<thissett.defect_bank["NMin4DB"]: pass
    else:
        ...
        for i in range(len(DefectBank_list)):
            isSame = DefectBank_list[i].is_same_structure(atoms_in, thissch_symbol, scaling=thissett.defect_bank["Scaling"], \
                      ignore_type=thissett.defect_bank["IgnoreType"], tolerance = thissett.defect_bank["Tol4Disp"])
            if isSame:
                thisdisps = DefectBank_list[i].load_disps(scaling=thissett.defect_bank["Scaling"], LoadRatio = thissett.defect_bank["Ratio4DispLoad"])
                break
    return thisdisps
    ...
    def get_ai_atomsquence(loadsett, thisAV, idav):
        ...
        for file in os.listdir(thispath):
            if thisfhead in file:
                ...
                coords = coords[:, 0:nn]
                for i in range(nn):
                    thisxyz = np.array([thisdf.at[thisdf.index[i], 'x'], thisdf.at[thisdf.index[i], 'y'], thisdf.at[thisdf.index[i], 'z']])
    ...
    if loadsett["Method"][0:4].upper() == "FILE":
        ...
        for file in os.listdir(thispath):
            if thisfhead in file:
                ...
                thisdisps = np.vstack((thisdf["dx"].to_numpy(), thisdf["dy"].to_numpy(), thisdf["dz"].to_numpy()))*loadsett["Ratio4DispLoad"]
