...
def calibrate_energy_with_DataSPs(thissett, DataSPs, seakmcdata, AVitags, Eground, object_dict, ReBias=False, float_precision=3):
    ...
    for iav in range(len(DataSPs.df_SPs)):
        ...
        for i in range(nSP):
            thisid = ispstart+i
            thisRecal = True
            if isinstance(thissett.saddle_point["Thres4Recalib"], float) or isinstance(thissett.saddle_point["Thres4Recalib"], int):
                if DataSPs.df_SPs[iav].at[i, "barrier"] - DataSPs.barriermin > thissett.saddle_point["Thres4Recalib"]: thisRecal = False
            if thisRecal:
                thisdata = copy.deepcopy(seakmcdata)
                thisdata.update_coords_from_disps(idav, DataSPs.disps[ispstart+i], AVitags)
                [thisesp, coords, isValid, errormsg] = force_evaluator.run_runner("MD0", thisdata, 0, nactive = thisdata.natoms, thisExports = None)
                if not isValid:
                ...
                if ReBias:
                    ...
                    [thisefin, coords, isValid, errormsg] = force_evaluator.run_runner("MD0", thisdata, 0, nactive = thisdata.natoms, thisExports = None)
                    if not isValid:
                        LogWriter.write_data(errormsg)
                        quit()
