...
from pymatgen.core.bonds import CovalentBond, bond_lengths, obtain_all_bond_lengths, get_bond_length
...
SP_DATA_HEADER = ["idav", "barrier", "prefactor", "ebias", "dmag", "dmax", "dsum", "adsum", "dmagfi", "dmaxfi", "dsumfi", "adsumfi", "isConnect"]
SP_KMC_HEADER = ["idav", "barrier", "prefactor", "ebias", "dmag", "dmax", "dsum", "dsumr", "dmagfi", "dmaxfi", "dsumfi", "dsumrfi", "isConnect", "probability", "isSel"]
SP_KMC_SELECTED_HEADER = ["isp", "barrier", "prefactor", "ebias", \
                          ...
                          "dmagxfs", "dmagyfs", "dmagzfs", "dmaxxfs", "dmaxyfs", "dmaxzfs", "vmaxxfs", "vmaxyfs", "vmaxzfs", \
                          "dsumxfs", "dsumyfs", "dsumzfs", "adsumxfs", "adsumyfs", "adsumzfs", \
                          "dtotfi", "dmagfi", "dmaxfi", "dsumfi", "adsumfi", \
                          "dmagxfi", "dmagyfi", "dmagzfi", "dmaxxfi", "dmaxyfi", "dmaxzfi", "vmaxxfi", "vmaxyfi", "vmaxzfi", \
...
export_Keys =["istep", "ground_energy", "nDefect", "defect_center_xs", "defect_center_ys", "defect_center_zs", \
              "nBasin", "nSP_thisbasin", "nSP_superbasin", "iSP_selected", "forward_barrier", "ebias", "backward_barrier", \
              "time_step",  "simulation_time"]

globals_dict={"significant_figures": 6, "float_precision": 3, "VerySmallNumber": 1.0e-20, "Tolerance": 0.1, "angle_tolerance": 5.0, "float_format": "%.3f"}
class Globals():
    def __init__(
        self,
        system,
...
class Settings():
    ...
    def __str__(self):
        keys=["system", "force_evaluator", "potential", "data", "kinetic_MC", "active_volume", "defect_bank", "dynamic_matrix", "spsearch", "saddle_point", "visual"]
        s = ""
        for key in keys:
            s += key + ":\n"
            if key == "system": s += str(self.system) + "\n"
            elif key == "force_evaluator": s += str(self.force_evaluator) + "\n"
            elif key == "potential": s += str(self.potential) + "\n"
            elif key == "data": s += str(self.data) + "\n"
            elif key == "kinetic_MC": s += str(self.kinetic_MC) + "\n"
            elif key == "active_volume": s += str(self.active_volume) + "\n"
            elif key == "defect_bank": s += str(self.defect_bank) + "\n"
            elif key == "dynamic_matrix": s += str(self.dynamic_matrix) + "\n"
            elif key == "spsearch": s += str(self.spsearch) + "\n"
            elif key == "saddle_point": s += str(self.saddle_point) + "\n"
            elif key == "visual": s += str(self.visual) + "\n"
...
#########################################################
        ...
        thisfeval={"Bin": "pylammps", "Path2Bin": False, "Style": "pylammps", "nproc": 1, "processors": False, "partition": False, \
            ...
            for i in range(len(thisfeval["OutFileHeaders"])-1, -1, -1):
                if not isinstance(thisfeval["OutFileHeaders"][i], str):
                    ...
                    if thisfeval["OutFileHeaders"][i].upper() == "NONE":
                        del thisfeval["OutFileHeaders"][i]
        else: thisfeval["OutFileHeaders"] = []


        if thisfeval["Style"].lower() == "pylammps": thisfeval["Bin"] = "pylammps"
        elif thisfeval["Style"].lower() == "lammps": thisfeval["Bin"] = "calllammps"
        elif thisfeval["Style"].lower() == "vasp": thisfeval["Bin"] = "callvasp"
        else: pass
############################################################
        potential = parameters['potential']
        symbols = []
        if 'species' not in potential:
            raise ValueError("Potential input must contain species.")
        else:
        ...
        ntype = len(symbols)
        if ntype < 1:
            raise ValueError("No species has been found.")
        ...
        if "OpenKIM" in potential:
            ...
            if OpenKIM["OpenKIM"]:
                if not isinstance(OpenKIM["kim_init"], str):
                    raise ValueError("kim_init must be a string!")
                if not isinstance(OpenKIM["kim_interaction"], str):
                    raise ValueError("kim_interaction must be a string!")
        ...
        if not OpenKIM["OpenKIM"]:
            ...
            if "pair_style" not in potential:
                if "LAMMPS" in thisfeval["Style"].upper():
                    raise ValueError("Potential input must contain pair_style.")
        else:
        ...
        if 'masses' in potential:
            for ii in range(len(potential['masses'])):
                ...
                try:
                    ...
                    if itype< ntype:
                        try: masses[itype] = float(thisstr[1].strip())
                        except: pass
                    else: pass
                except: pass
        ...
        if 'cutneighmax' in potential:
            try: cutneighmax = float(potential['cutneighmax'])
            except: pass
...
###################################################################
        ...
        thisav ={"NPredef": 0, "PredefOnly": False, "Style": "defects", \
                 ...
                 "RT_SetMolID": False, "DefectCenter4RT_SetMolID": "AUTO", "R4RT_SetMolID": 30, "FCT4RT_SetMolID": ["INF", "INF", "INF", "INF", "INF", "INF"], \
                 "NMax4Def": False, "NMax4AV": False, "NMin4AV": 40, \
                 "PDReduction": True, "SortD4PDR": False, "DCut4PDR": cutdefectmax*1.4, "RecursiveRed": False, "Order4Recursive4PDR": None, \
                 "DCut4noOverlap": 9.0*cutdefectmax, "Overlapping": True, "Order4Recursive4AV": None, "Overlap4OrderRecursive": True , \
                 "Stack4noOverlap": False, "PointGroupSymm": False, "NMax4PG": 1000, \
                 "Sorting": True, "Sort_by": sort_by, "SortingSpacer": [0.3, 0.3, 0.3],  "SortingShift": [0.0, 0.0, 0.0], \
        ...
        if active_volume["Style"].upper() == 'CUSTOM':
            if "NActive" not in active_volume:
                raise ValueError("Must input NActive for custom style of constructing AV!")
            else:
            ...
            if "Method" in active_volume["FindDefects"]:
                if "WS" in active_volume["FindDefects"]["Method"].upper():
                    if "ReferenceData" not in active_volume["FindDefects"]:
                        raise ValueError("Must have 'ReferenceData' for WS method of FindDefects!")
                    ...
                    if len(defects)<=0:
                        raise ValueError("Must input coordinates of defects for custom FindDefect!")
                else:
                    if "DCut4Def" not in active_volume["FindDefects"]:
        ...
        for key in active_volume:
            if key=="FindDefects":
                ...
                if isinstance(active_volume[key], list):
                    ...
                    if len(t)>0: thisav[key] = t[0:len(t)]
                    else: thisav[key] = sort_by
...
#######################################################
        ...
        HandleVN = {"CheckAng4Init": True, "AngTol4Init": 5.0, "MaxIter4Init": 20, "NMaxRandVN": 20, \
                    ...
                    "RescaleStyle4LOGV": "SIGMOID", "Period4MA": 1, "XRange4LOGV": 20.0, "PowerOnV": 4, "Ratio4Zero4LOGV": 0.2, "MinValue4LOGV": -20.0, \
        ...
        thisspsearch={"Method":"dimer", "NSearch": 10, "SearchBuffer": False, "NMax4Trans": 1000, "IgnoreSteps": 4, \
                "DimerSep": 0.005, "NMax4Rot": 3, "FThres4Rot": 0.1,  "FMin4Rot": 0.001, "TrialStepsize": 0.015, "MaxStepsize": 0.05, \
                "DRatio4Relax": 2.0, "FConv": 1e-6, "EnConv": 1e-5, "En4TransHorizon": 0.1, "TransHorizon": True, \
                ...
                "ShowIterationResults": False, "Inteval4ShowIterationResults": 1, "ShowVN4ShowIterationResults": False, "ShowCoords4ShowIterationResults": False, \
                "ApplyMass": False, "TaskDist": "AV", "Master_Slave": True, \
                "LocalRelax": LocalRelax, "force_evaluator": spsearch_force_evaluator, "Preloading": Preloading, "HandleVN": HandleVN}
...
################################################################################
        thisKMC = {"NSteps": 1, "Temp": 800.0, "Temp4Time": 800.0, "AccStyle": "NoAcc", "NMaxBasin": "NA", "Tol4Disp": 0.1, "Tol4Barr": 0.03, \
        ...
        if "kinetic_MC" in parameters:
            ...
            if "Temp4Time" not in KMC:
                try: thisKMC["Temp4Time"] = KMC["Temp"]
                except: pass
#################################################################################
...
#################################################################################
        thisDefectBank = {"Preload": False, "NMax4DB": 200, "NMin4DB": 40, \
                      ...
                      "Recycle": False, "UseSymm": False, "SaveDB": False, "SavePath": "DefectBank", "OutIndex": True}
...
#################################################################################
        ...
        ScreenEng = {"AND4ScreenE":[True], "Type4ScreenE": ["barrier"], "MinVal4ScreenE": ["NA"], "MaxVal4ScreenE": ["NA"]}
        ValidSPs = {"RealtimeValid": False, "RealtimeDelete": False, "CheckConnectivity": True, \
                    ...
                    "FindSPType": False, "EnCut4Type": 0.05, "MagCut4Type": 0.05, "LenCut4Type": 0.05, "AngCut4Type": 5.0, \
                    "MaxRatio4Dmag": "NA", "MaxRatio4Barr": "NA", \
                    "EnTol4AVSP": 0.1, "Tol4AVSP": 0.1, "NMax4Dup": 600, "NCommonMin": 10, "R2Dmax4Tol": 0.1, "Tol4Disp": 0.1}
        ...
        nScreenDisp = thissp["ValidSPs"]["NScreenDisp"]
        nScreenEng = thissp["ValidSPs"]["NScreenEng"]
        if nScreenDisp>0:
...
##############################################################################
        Write_Data_SPs = {"Write_Data_SPs": True, "DispStyle4DataSP": "Both", "OutputStyle": "SEP", "Sel_iSPs": "AUTO", "Offset": 0, \
                          "Write_Data_AVs": True, "Write_KMC_Data": True, "Write_Prob": True, "DetailOut": True, "SPs4Detail": "AUTO"}
        Write_AV_SPs= {"Write_Local_AV": False, "Write_AV_SPs": False, "Write_Data_AV_SPs": False, "DispStyle4AVSP": "SP"}
        thisvisual ={"Screen": True, "Log": True, "Write_SP_Summary": True, \
                     "RCut4Vis": 0.04, "DCut4Vis": 0.01, "Invisible": True, "Reset_Index": False, "ShowBuffer": False, "ShowFixed": False, \
        ...
        if "visual" in parameters:
            ...
            if "Write_Data_SPs" in visual and "Sel_iSPs" in visual["Write_Data_SPs"]:
                ...
                if SelSPs.upper() == "AUTO": pass
                elif SelSPs.upper() == "ALL": pass
                elif SelSPs.upper() == "TRANSIENT": pass
                elif SelSPs.upper() == "ABSORB": pass
                else:
                    tmpl = []
                    tmps = SelSPs.split(",")
                    for i in range(len(tmps)):
                        try: tmpl.append(int(tmps[i].strip()))
                        except: pass
        ...
        thissett = cls(thissystem, thisfeval, potential, thisdata, thisKMC, thisav, thisDefectBank, thisDynMat, thisspsearch, thissp, thisvisual)
    ...
    def validate_input(self):
        ...
        if not self.active_volume["Overlapping"]:
            if self.spsearch["SearchBuffer"]:
                if self.active_volume["DCut4noOverlap"]<= self.active_volume["DActive"] + self.active_volume["DBuffer"]:
                    raise ValueError("DCut4noOverlap must be larger than DActive+DBuffer!")
            else:
                if self.active_volume["DCut4noOverlap"]<= self.active_volume["DActive"]:
                    raise ValueError("DCut4noOverlap must be larger than DActive!")

        if self.kinetic_MC["AccStyle"][0:3].upper()=="MRM":
            if isinstance(self.kinetic_MC["EnCut4Transient"], float) or isinstance(self.kinetic_MC["EnCut4Transient"], int):
                pass
            else:
                raise ValueError("EnCut4Transient in kineticMC must be float number!")
        if self.saddle_point["CalEbiasInData"] and self.saddle_point["CalBarrsInData"] is False:
            raise ValueError("CalBarrsInData in saddle_point must be True if CalEbiasInData is True!")

        if self.spsearch["MaxStepsize"] <= self.spsearch["TrialStepsize"]:
            raise ValueError("The MaxStepsize must be larger than TrialStepsize!")
        ...
        if isinstance(self.spsearch["FixTypes"], list):
            ...
            self.spsearch["FixTypes_dict"] = None
