...
class VaspRunner(object):
    ...
    def run_runner(purpose, data, thiscolor, nactive = None, thisExports = None):
    ...
    def preparation(self, purpose, data, thiscolor, nactive = None, thisExports = None, nproc = 1):
        if purpose == "DATAMD":
            ...
            if isinstance(self.sett.spsearch["force_evaluator"]["RinputOpt"], str):
                rinputs = self.read_input_script(self.sett.spsearch["force_evaluator"]["RinputOpt"], thiscolor)
            else:
                rinputs = self.get_default_inputs(purpose, data, thiscolor, nactive = nactive)
            shutil.copy("KPOINTS_SPS", self.relative_path+"/KPOINTS")
    ...
    def get_default_inputs(self, purpose, data, thiscolor, nactive = None):
        ...
        if purpose == "DATAMD": pass
        elif purpose == "DATAMD0": pass
        elif purpose == "DATAOPT" or purpose == "DATARELAX": pass
        elif purpose == "MD0" or "RECAL" in purpose: pass
        elif purpose == "OPT" or purpose == "RELAX": pass
        elif purpose == "SPSDYNMAT" or purpose=="DYNMAT": pass
        elif purpose == "SPS": pass
        elif purpose == "SPSOPT" or purpose == "SPSRELAX": pass
            ...
            for i in range(len(rinputs)):
                ...
                for ik in range (n):
                    ...
                    if istart != -1:
                        ...
                        else: newline = thislines[0]+ delimiter + key + " " + str(InVals[ik])
                        if na > 1:
                            for j in range(1, na):
                                newline += delimiter+afters[j]
                        rinputs[i] = newline
            return rinputs
        else:
            return rinputs
