...
from lammps.constants import LMP_STYLE_GLOBAL, LMP_STYLE_ATOM, LMP_STYLE_LOCAL, LMP_TYPE_SCALAR, LMP_TYPE_VECTOR
from lammps.constants import LMP_TYPE_ARRAY, LMP_SIZE_VECTOR, LMP_SIZE_ROWS, LMP_SIZE_COLS
...
class PyLammpsRunner(object):
    ...
    def run_runner(self, purpose, data, thiscolor, nactive = None, thisExports = []):
        ...
        rinputs = self.preparation(purpose, data, thiscolor, nactive=nactive, \
                           thisExports = thisExports,  nproc=nproc_task)
        ...
        if total_energy is None or relaxed_coords is None:
            isValid = False
            errormsg = f"Error on running PYLAMMPS - purpose: {purpose} datatype:{type(data)} thiscolor: {thiscolor} nactive:{nactive}!"
    ...
    def init_spsearch_runner(self, data, thiscolor, nactive):
        ...
        data.to_lammps_data(self.relative_path+"/tmp0.dat", to_atom_style=True, distance=12)
        rinputs = self.preparation(purpose, data, thiscolor, nactive=nactive, nproc = nproc_task)
    ...
    def init_binary(self, nproc = 1, comm=None, Screen = False, Log = False):
        ...
        if isinstance(Log, str): args += ["-log", Log]
        else: args += ["-log", "none"]
    ...
    def preparation(self, purpose, data, thiscolor, nactive = None, thisExports = [],  nproc =1):
        purpose=purpose.upper()
        if nactive is None:
            try: nactive = data.nactive
            except: nactive = data.natoms
        if purpose == "DATAMD":
            ...
            if isinstance(self.sett.spsearch["force_evaluator"]["RinputDM"], str):
                rinputs = self.modify_input_script(self.sett.spsearch["force_evaluator"]["RinputDM"], thiscolor, nactive)
            else:
                rinputs = self.get_default_inputs(purpose, data, thiscolor, nactive)
        elif purpose == "SPS":
            ...
            if isinstance(self.sett.spsearch["force_evaluator"]["RinputOpt"], str):
                rinputs = self.modify_input_script(self.sett.spsearch["force_evaluator"]["RinputOpt"], thiscolor, nactive)
            else:
                rinputs = self.get_default_inputs(purpose, data, thiscolor, nactive)
    ...
    def get_default_inputs(self, purpose, data, thiscolor, nactive):
        ...
        if purpose == "DATAMD":
            ...
            if self.sett.force_evaluator["Relaxation"]["InitTemp4Opt"]>self.sett.force_evaluator["Relaxation"]["TargetTemp4NVT"]:
                lines.append("velocity     all create %f 10 dist uniform" %(self.sett.force_evaluator["Relaxation"]["InitTemp4Opt"]))
                t = self.sett.force_evaluator["Relaxation"]["TargetTemp4NVT"]
                lines.append("fix 1nvt all nvt   temp  %f  %f   %f " % (t, t, 0.33*self.sett.force_evaluator["Relaxation"]["InitTemp4Opt"]))
            ...
            lines.append("dynamical_matrix gactive regular %f file %s " %(self.sett.dynamic_matrix["displacement"], "Runner_" + str(thiscolor)+"/"+self.dynmat))
            lines.append("run   0")
            #lines.append("print   etotal=$(pe+ke)")
    ...
    def modify_input_script(self, input_file, thiscolor, nactive):
        ...
        for i in range(len(lines)):
            ...
            if "dynamical_matrix " == lines[i][0:17]:
                lines[i] = "dynamical_matrix gactive regular %f file %s " %(self.sett.dynamic_matrix["displacement"], "Runner_" + str(thiscolor)+"/"+self.dynmat)
    ...
    def ImportValue4RinputOpt(self, rinputs, thisExports=[]):
        ...
        if thisExports is None: isValid = False
        elif len(thisExports) == 0: isValid = False
        if isValid:
            ...
            for i in range(len(rinputs)):
                ...
                for ik in range (n):
                    ...
                    if istart != -1:
                        ...
                        na = len(afters)
                        if istart == 0: newline = key + " " + str(InVals[ik])
                        else: newline = thislines[0]+ " " + key + " " + str(InVals[ik])
                        if na > 1:
                            for j in range(1, na):
                                newline += " "+afters[j]
                        rinputs[i] = newline
