...
from seakmc.input.Input import SP_COMPACT_HEADER, SP_COMPACT_HEADER4Delete, SP_DATA_HEADER
from seakmc.core.data import SeakmcData
from seakmc.restart.Restart import RESTART
from seakmc.kmc.KMC import Basin, SuperBasin
...
def run_seakmc(thissett, seakmcdata, object_dict, Eground, thisRestart):
    ...
    if thisRestart is None:
        ...
        DefectBank_list = []
        if thissett.defect_bank["LoadDB"]: DefectBank_list = preSPS.load_DefectBanks(thissett.defect_bank, out_paths[2], significant_figures=thissett.system["significant_figures"])
        istep_this = 0
    else:
    ...
    if isinstance(thissett.active_volume["DefectCenter4RT_SetMolID"], list): last_de_center = thissett.active_volume["DefectCenter4RT_SetMolID"]
    else: last_de_center = None
    ...
    for istep in range(istep_this, thissett.kinetic_MC['NSteps']):
        ...
        if thisRestart is None:
            ...
            logstr += "\n" + f"The fractional coords of the defect center are {np.around(seakmcdata.de_center, decimals=thissett.system['float_precision'])} at {istep} KMC step!"
            logstr += "\n"
            LogWriter.write_data(logstr)
        else:
        ...
        seakmcdata, DataSPs, AVitags = dataSPS.data_find_saddlepoints(istep, thissett, seakmcdata, DefectBank_list, thisSuperBasin, Eground, \
        ...
        if thissett.saddle_point["CalBarrsInData"]:
            DataSPs, df_delete_this = myRecal.calibrate_energy_with_DataSPs(thissett, DataSPs, seakmcdata, AVitags,  \
                            Eground, object_dict, ReBias=thissett.saddle_point["CalEbiasInData"], float_precision=thissett.system["float_precision"])
        ...
        simulation_time, thiskmc, thisSuperBasin, thisExports = dataKMC.run_KMC(istep, thisSuperBasin, \
                       seakmcdata, AVitags, DataSPs, thissett, simulation_time, thisExports, LogWriter)

        thisSummary.update_data(thisExports)
        thisSummary.write_data()
        this_simulation_time = thiskmc.timeelapse
        dataout.write_prob_to_file(thissett.visual, thiskmc, DataSPs, istep, out_paths[4], VerySmallNumber=thissett.system["VerySmallNumber"])
        sel_SPs = dataout.get_sel_SPs_for_out(thissett.visual, thiskmc, DataSPs)
        if len(sel_SPs)>0: dataout.visualize_data_SPs(thissett.visual, seakmcdata, AVitags, DataSPs, sel_SPs, istep, out_paths[1])
        else: dataout.visualize_data_SPs_Superbasin(thissett.visual, thiskmc, thisSuperBasin, istep, out_paths[1])
        DataSPs = None
        sel_SPs = None
        ...
        [Eground, relaxed_coords, isValid, errormsg] = force_evaluator.run_runner("OPT", seakmcdata, 0, nactive = seakmcdata.natoms, thisExports = thisExports)
        if not isValid:
        ...
        seakmcdata.to_atom_style()
        seakmcdata.velocities=None
        if thissett.visual["Write_Data_SPs"]["Write_KMC_Data"]:  seakmcdata.to_lammps_data(out_paths[1]+"/"+"KMC_"+str(istep+1)+".dat", to_atom_style=True)
