...
def initial_data_dynamics(thissett, seakmcdata, force_evaluator, LogWriter):
    ...
    if thissett.data["MoleDyn"]:
        ...
        [Eground, relaxed_coords, isValid, errormsg] = force_evaluator.run_runner("DATAMD", seakmcdata, thiscolor, nactive = seakmcdata.natoms, thisExports = None)
        if not isValid:
        ...
        [Eground, relaxed_coords, isValid, errormsg] = force_evaluator.run_runner("DATAOPT", seakmcdata, thiscolor, nactive = seakmcdata.natoms, thisExports = None)
        if not isValid:
            LogWriter.write_data(errormsg)
            quit()
        seakmcdata = SeakmcData.from_file("Runner_0/tmp1.dat", atom_style = thissett.data['atom_style_after'])
        ...
        [Eground, relaxed_coords, isValid, errormsg] = force_evaluator.run_runner("DATAMD0", seakmcdata, thiscolor, nactive = seakmcdata.natoms, thisExports = None)
        if not isValid:
            LogWriter.write_data(errormsg)
            quit()        
...
def preprocess(thissett):
    ...
    if thisRestart is None:
        ...
        if thissett.visual["Write_Data_SPs"]["Write_KMC_Data"]:  seakmcdata.to_lammps_data(out_paths[1]+"/"+"KMC_"+str(0)+".dat", to_atom_style=True)
