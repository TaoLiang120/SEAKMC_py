    ...
    def write_data(self):
        ...
        for i in range(1, len(export_Keys)):
            if export_Keys[i]=="nDefect"  or export_Keys[i]=="nSP_thisbasin" or \
               export_Keys[i]=="nSP_superbasin" or export_Keys[i]=="nBasin" or export_Keys[i]=="iSP_selected":
...
def object_maker(thissett, thisRestart):
    ...
    thisLogWriter = LogWriter(THIS_PATH, Screen = thissett.visual["Screen"], Log = thissett.visual["Log"], RESTART=thisRestart)
    object_dict['LogWriter'] = thisLogWriter

    thisSummary = SeakmcSummary(THIS_PATH, RESTART=thisRestart, significant_figures = thissett.system["significant_figures"])
